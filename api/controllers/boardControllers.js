...
const boardGet = async (req, res) => {
  try {
    const data = await Board.find().populate("userId"); // Populate the username field from User
...
const boardPatch = async (req, res) => {
  try {
    ...
    if (description && typeof description !== "string")
      return res.status(400).json({ data: "Invalid description: Wrong Type" });
    if (visibility && !enums.visibilityEnum.includes(visibility))
      return res.status(400).json({ message: "Invalid visibility value." });
